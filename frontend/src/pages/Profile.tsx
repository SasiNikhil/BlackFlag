...
export default function Profile() {
  ...
  const employee = useMemo(() => {
    ...
    const byEmpId = employees.find(e => e.employee_id === user.employee_id)
    if (byEmpId) return byEmpId
    return initialEmployees.find(e => e.email.toLowerCase() === user.email.toLowerCase() || e.employee_id === user.employee_id)
  }, [employees, user])

  const employeeKeySet = useMemo(() => {
    const keys = new Set<string>()
    if (employee?.id) keys.add(employee.id.toLowerCase())
    if (employee?.employee_id) keys.add(employee.employee_id.toLowerCase())
    return keys
  }, [employee])

  const myBalances = employee ? leaveBalances.filter(b => employeeKeySet.has(b.employee_id.toLowerCase())) : []
  const myDocuments = employee ? documents.filter(d => employeeKeySet.has(d.employee_id.toLowerCase())) : []
