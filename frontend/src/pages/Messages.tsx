...
export default function Messages() {
  const { user, employees, messages, sendMessage, markAsRead, getConversation } = useApp()
  ...
  const currentUserId = user?.id || 'emp-001'
  ...
  // Filter conversations by search query OR show all employees if searching
  const filteredConversations = useMemo(() => {
    if (!searchQuery.trim()) return conversations
    
    const query = searchQuery.toLowerCase()
    
    // Show ALL matching employees from directory, not just existing conversations
    const matchingEmployees = employees
      .filter(emp => {
        if (emp.id === currentUserId) return false // Exclude self
      ...
      .map(emp => {
        ...
        const existingConv = conversations.find(c => c.participant_id === emp.id)
        if (existingConv) return existingConv
        
        // Create placeholder conversation for employees not yet messaged
        return {
  ...
  }, [conversations, searchQuery, employees, currentUserId])
