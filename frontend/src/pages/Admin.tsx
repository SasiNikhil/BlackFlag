...
export default function Admin() {
  const { employees, user, leaveRequests, documents, addNotification } = useApp()
  const [showModal, setShowModal] = useState(false)
  const [activeTab, setActiveTab] = useState<'overview' | 'employees' | 'reports'>('overview')

  // Only HR admin can access admin panel
  if (!user || user.role !== 'hr_admin') {
    return (
      <div className="text-center py-12 text-stone-500">
        <p className="text-lg font-medium">Access Denied</p>
        <p className="text-sm">Only HR Administrators can access the admin panel.</p>
      </div>
    )
  }
  ...
  const stats = useMemo(() => {
    ...
    return { active: active.length, pending: pending.length, byDept, docs: documents.length }
